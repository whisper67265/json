...
[NOTE]
<<ref_value_to>> does not have a similar parameter, as <<ref_value>> is not created.
...
For example, consider this alternative implementation of `tag_invoke` for `ip_address` from the section <<custom_conversions>>.
