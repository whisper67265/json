...
[#dom]
= Document Model In this library the following types implement containers used to represent JSON data in memory:
...
access. The interface and performance characteristics are similar to {std_vector}.

| <<ref_object>>
| An associative container of key-value pairs with unique keys, where the key
is a string and the mapped type is a JSON value. Search, insertion, and removal have average contant-time complexity. In addition, elements are stored contiguously in memory allowing cache-friendly iteration.
