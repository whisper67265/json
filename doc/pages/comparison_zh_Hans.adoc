...
which rules out the most important type of allocator, a local arena-based implementation.
...
performing `a = b` is equivalent to `a = std::move(b)`. No copy construction or copy assignment allowed.
